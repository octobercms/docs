...
In some cases it makes sense to retrieve a column value from a nested data structure, such as a [model relationship](../database/relations.md) column or a [jsonable array](../database/model.md#oc-property-jsonable). The only drawback of doing this is the column cannot use searchable or sortable options.
