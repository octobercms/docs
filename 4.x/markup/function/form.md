...
Outputs a standard `<form>` opening tag along with the `_session_key` and `_token` hidden fields for CSRF protection. If you are using the [AJAX Framework](../../ajax/introduction.md), it is recommended that you use `form_ajax()` instead.
...
**request** | A handler name to execute on the server when the form is posted. See the [AJAX handlers](../../ajax/handlers.md) article for details about event handlers.
