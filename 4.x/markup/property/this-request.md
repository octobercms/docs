...
To check if the current request was made using the [Turbo Router](../../ajax/turbo-router.md), access the `this.request.pjax` property.
...
Use the `this.request.pjaxCached` property to also check if the [Turbo Router](../../ajax/turbo-router.md) request had a cached snapshot coming before it.
