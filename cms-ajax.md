...
- **data-request-error** - specifies JavaScript code to execute if the request encounrters an error. Inside the JavaScript code you can access the following variables: `$el` (the page element triggered the request), the `context` object, the `textStatus` text string, and the `jqXHR` object.
