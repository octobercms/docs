...
Sometimes you may need to prevent a field from being submitted. In order to do that, just ad an underscore (\_) before the name of the field in the form configuration file. Form fields beginning with an underscore are purged automatically and no longer saved to the model.
