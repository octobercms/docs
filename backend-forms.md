...
Option | Description
------------- | -------------
**stretch** | this will stretch the form field box to cover the height of the rest of the form field area, **make sure there are no other form fields below it as it needs to be the last form field**, otherwise you will see overflow errors. We recommend you using this feature on multiline inputs like `richeditor`, `markdown`, `textarea`, `partial` or `hint`. A value of `1` will turn on the feature and a vlue of `0` will turn it off.
**grow** | this will make the form field have dynamic height and grow when more content is added. We recommend you using this feature on multiline inputs like `richeditor`, `markdown` or `textarea`. A value of `1` will turn on the feature and a vlue of `0` will turn it off.
