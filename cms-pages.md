...
All websites have pages. In October pages are represented with page templates. Page template files reside in the **/pages** subdirectory of a theme directory. Page file names do not affect the routing, but it's a good idea to name your pages accordingly to the page function. The files should have the **htm** extension. The [Configuration](../master/cms-themes.md#configuration-section) and [Twig](../master/cms-themes.md#twig-markup-section) template sections are required for pages but the [PHP section](../master/cms-themes.md#php-code-section) is optional. Below you can see the simplest home page example.
...
Page configuration is defined in the [Configuration Section](../master/cms-themes.md#configuration-section) of the page template file. The page configuration defines the page parameters, required for the routing and rendering the page and page [Components](../master/cms-components.md), which are explained in another article. The following configuration parameters are supported for pages:

- **url** - the page URL, required. The URL syntax is described below.
- **title** - the page title, required.
- **layout** - the page [layout](../master/cms-layouts.md), optional. If specified, should contain the name of the layout file, without extension, for example: `default`.
...
URLs with parameters are more flexible. A page with the URL pattern defined in the following example would be displayed for any address like `/blog/post/something`. URL parameters can be accessed by October components or from the page [PHP code](../master/cms-themes.md#php-code-section) section.
...
More information can be found on [default variables in the Markup guide](../master/cms-markup.md#default-variables).

<a name="dynamic-pages" class="anchor" href="#dynamic-pages"></a>
## Dynamic pages

Inside the [Twig section](../master/cms-themes.md#twig-markup-section) of a page template you can use any native [Twig](http://twig.sensiolabs.org/documentation) functions, filters and tags as well as the [Twig extensions provided by October](../master/cms-themes.md#twig-markup-section). Any dynamic page requires **variables**. In October page variables can be prepared by the page or layout [PHP section](../master/cms-themes.md#php-code-section) or by [Components](../master/cms-components.md). In this article we describe how to prepare variables in the PHP section.

<a name="page-cycle-handlers" class="anchor" href="#page-cycle-handlers"></a>
### Page execution cycle handlers

There are special functions that can be defined in the PHP section of pages and layouts: `onInit()`, `onStart()` and `onEnd()`. The `onInit()` function is executed when all components are initialized and before AJAX requests are handled. The `onStart()` function is executed in the beginning of the page execution. The `onEnd()` function is executed before the page is rendered and after the page [components](../master/cms-components.md) are executed. In the onStart and onEnd functions you can inject variables to the Twig environment. Use the `array notation` to pass variables to the page:
...
The default variables and Twig extensions provided by October are described in the [Markup Guide](../master/cms-markup.md).

<a name="handling-forms" class="anchor" href="#handling-forms"></a>
### Handling forms

You can handle standard forms with handler methods defined in the page or layout [PHP section](../master/cms-themes.md#php-code-section) (handling the AJAX requests is explained in the [AJAX Framework](ajax) article). Use the [form_open()](markup#standard-form) function to define a form that refers to an event handler. Example:
...
The onHandleForm function can be defined in the page or layout [PHP section](../master/cms-themes.md#php-code-section) in the following way:
...
> **Note:** If a handler with a same name is defined in the page layout, page and a page [component](../master/cms-components.md) October will execute the page handler. If a handler is defined in a component and a layout, the layout handler will be executed. The handler precedence is: page, layout, component.

If you want to refer to a handler defined in a specific [component](../master/cms-components.md), use the component name or alias in the handler reference:
...
If needed, you can inject assets (CSS and JavaScript files) to pages with the controller's `addCss()` and `addJs()` methods. It could be done in the `onStart()` function defined in the [PHP section](../master/cms-themes.md#php-code-section) of a page or [layout](../master/cms-layouts.md) template. Example:
...
In order to output the injected assets on pages or [layouts](../master/cms-layouts.md) use the [{% styles %}](../master/cms-markup.md#injecting-css-links) and [{% scripts %}](../master/cms-markup.md#injecting-javascript-scripts) tags. Example:
