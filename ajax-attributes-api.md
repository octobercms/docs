...
**data-request-hide-while-loading** | specifies a CSS selector for an element to be hidden while the request runs. If you had an input with data-track-input you might use this option to hide the elements like the submit button until the ajax request is complete. The feature uses jQuery's `show()` and `hide()` functions to manage the element visibility.
