...
**data-request-ajax-global** | ``false`` by default. Set ``true`` for enable default jQuery [ajax events](http://api.jquery.com/category/ajax/global-ajax-event-handlers/): ``ajaxStart``, ``ajaxStop``, ``ajaxComplete``, ``ajaxError``, ``ajaxSuccess`` and ``ajaxSend``.
